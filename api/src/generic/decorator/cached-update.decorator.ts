...
export function CachedUpdate<TResult, TArgs extends unknown[] = unknown[]>(
    ...
    return function (
        ...
        descriptor.value = async function (...args: TArgs): Promise<TResult> {
            ...
            const updated = await cacheService.set(cacheKey, result, ttl)

            logger.debug(`${updated ? 'Updated' : 'Not found'} cache: ${cacheKey}`)
